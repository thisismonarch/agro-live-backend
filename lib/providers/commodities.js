...
async function fromCommodityPriceApi(key) {
  ...
  const out = {};
  const errors = {}; // agroKey -> reason, surfaced in /api/market meta so failures are never invisible again
  ...
  //
  // Auth: use ?apiKey=... in the URL, NOT an x-api-key header. Confirmed by
  // direct testing -- the query-param form is what actually worked against a
  // real key; their docs claim both work, but we've now hit real/documented
  // behavior mismatches twice with this API, so we go with what's proven.
  for (const [agroKey, sym] of Object.entries(wanted)) {
    try {
      const url = `https://api.commoditypriceapi.com/v2/rates/latest?symbols=${encodeURIComponent(sym)}&quote=USD&apiKey=${encodeURIComponent(key)}`;
      const data = await fetchJson(url);

      if (!data || data.success !== true || !data.rates) {
        errors[agroKey] = (data && (data.message || data.error)) || "bad or unsuccessful response";
        continue;
      }

      const raw = data.rates[sym];
      if (typeof raw !== "number" || raw <= 0) {
        errors[agroKey] = `symbol "${sym}" missing from response`;
        continue;
      }

      const meta = data.metaData || data.metadata || {};
      const symMeta = meta[sym] || {};
      if (!isUsdDenominated(symMeta.quote)) {
        errors[agroKey] = `non-USD quote (${symMeta.quote}) -- skipped rather than guess an FX conversion`;
        continue;
      }
      ...
      if (unitStr && UNIT_STRING_TO_KG[unitStr]) {
        ...
        const convert = COMMODITIES_API_CONVENTIONS[agroKey];
        if (!convert) { errors[agroKey] = "no conversion available for this commodity"; continue; }
        pricePerKg = convert(raw);
      }
      if (Number.isFinite(pricePerKg) && pricePerKg > 0) {
        out[agroKey] = pricePerKg;
      } else {
        errors[agroKey] = `conversion produced invalid price (raw=${raw})`;
      }
    } catch (err) {
      // this symbol failed (e.g. expired dated contract, network error) --
      // skip it, keep curated, don't break the rest, but remember why.
      errors[agroKey] = err.message;
    }
  }
  if (Object.keys(errors).length > 0) {
    // Always visible in Railway's logs (Deployments -> View Logs), even on
    // partial success -- so "why didn't X show up live" never requires
    // guessing again.
    console.warn("commoditypriceapi per-symbol issues:", JSON.stringify(errors));
  }
  if (Object.keys(out).length === 0) {
    throw new Error("commoditypriceapi: no prices -- " + JSON.stringify(errors));
  }
  return out;
}
