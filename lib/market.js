...
async function getCommodityPrices() {
  try {
    return { prices: await cache.remember("commodities", num("COMMODITY_TTL_MINUTES", 360), fetchCommodityPrices), error: null };
  } catch (err) {
    // Surface the real reason in /api/market's meta instead of swallowing it --
    // this is what let us debug the last commoditypriceapi issue without
...
async function assembleMarket() {
  ...
  const { prices: livePrices, error: commodityError } = await getCommodityPrices();
  if (commodityError) meta.commodityError = commodityError;
