...
// benchmarks (back-solved from a live API sample), not assumed.
//
// The API's /latest response can also include a "unit" object naming the
// unit per symbol (e.g. "per cwt", "per metric ton"). When present, we prefer
// it over our hardcoded table below — it's authoritative. When absent (some
// plans/endpoint versions omit it), we fall back to the verified table.
//
